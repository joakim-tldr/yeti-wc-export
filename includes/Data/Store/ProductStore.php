...
class ProductStore {
    /**
     * Resolve product and variation IDs according to filters and requested fields.
     * This mirrors existing Exporter logic but is not wired-in yet (kept for future switch-over).
     */
    public function resolveItemIds(array $filters, array $fields): array {
        ...
        // Normalize product status: allow 'any' string or array of statuses
        ...
        if (is_wp_error($product_ids)) return ['ids' => [], 'total' => 0];
        $ids = $product_ids;
        ...
        if ($needs_variations) {
            ...
            if (!in_array('variable', $product_types, true) && !in_array('all', $product_types, true)) {
                ...
                if (!empty($args['date_query'])) $var_parent_args['date_query'] = $args['date_query'];
            ...
            if (!empty($parents)) {
                $variation_args = [
                    'post_type' => 'product_variation', 'posts_per_page' => -1, 'fields' => 'ids',
                    'post_parent__in' => $parents, 'post_status' => $args['post_status'],
                ];
                ...
                if (!is_wp_error($variation_ids) && !empty($variation_ids)) {
                    ...
                    foreach ($variation_ids as $vid) {
                        ...
                        if ($parent) { $variation_map[$parent][] = $vid; }
                    }
                    $ordered = []; $seen = [];
                    foreach ($product_ids as $pid) {
                        $ordered[] = $pid; $seen[$pid] = true;
                        if (!empty($variation_map[$pid])) {
                            foreach ($variation_map[$pid] as $vid) { $ordered[] = $vid; $seen[$vid] = true; }
                            unset($variation_map[$pid]);
                        }
                    }
                    foreach ($variation_map as $vids) {
                        foreach ($vids as $vid) if (empty($seen[$vid])) { $ordered[] = $vid; $seen[$vid] = true; }
                    }
                    $ids = $ordered;
                }
            }
