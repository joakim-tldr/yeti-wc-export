...
class YWCE_Exporter {
	...
	public function start_export(): void {
		...
		try {
			...
			if ($data_source === 'product') {
				...
				// Build product query
				$this->debug_log('YWCE: Final query args: ' . print_r($args, true));
				$product_ids = get_posts($args);
				
				if (is_wp_error($product_ids)) {
					wp_send_json_error(['message' => 'Error retrieving products: ' . $product_ids->get_error_message()]);
					return;
				}
				...
				// Include variations whenever either Parent ID or plytix_variant_of is selected.
				// This mirrors the Preview behavior and ensures these fields are populated in exports.
				$needs_variations = in_array( 'Parent ID', $fields, true ) || in_array( 'plytix_variant_of', $fields, true );
				if ( $needs_variations ) {
					// Start with the current product IDs as potential parents
					$parent_ids_for_variations = $product_ids;

					// If the current product type filter excludes variable products,
					// we still need to include variations. Fetch variable parent IDs using the same
					// status and date filters so variation rows can be exported.
					if ( ! in_array( 'variable', $product_types, true ) && ! in_array( 'all', $product_types, true ) ) {
						$variable_parent_args = [
							'post_type'      => 'product',
							'posts_per_page' => -1,
							'fields'         => 'ids',
						];
						...
						// Apply status filter
						if ( ! empty( $product_status ) && ! in_array( 'all', $product_status, true ) ) {
							$variable_parent_args['post_status'] = $product_status;
						} else {
							$variable_parent_args['post_status'] = 'any';
						}

						// Restrict to variable product type
						$variable_parent_args['tax_query'] = [
							[
								'taxonomy' => 'product_type',
								'field'    => 'slug',
								'terms'    => [ 'variable' ],
						...
						// Apply the same date filters if used
						if ( $export_mode !== 'all' && ! empty( $args['date_query'] ) ) {
							$variable_parent_args['date_query'] = $args['date_query'];
						}

						$this->debug_log( 'YWCE: Fetching variable parent IDs with args: ' . print_r( $variable_parent_args, true ) );
						$variable_parents = get_posts( $variable_parent_args );
						if ( ! is_wp_error( $variable_parents ) && ! empty( $variable_parents ) ) {
							$parent_ids_for_variations = array_unique( array_merge( $parent_ids_for_variations, $variable_parents ) );
						}
					}

					if ( ! empty( $parent_ids_for_variations ) ) {
						$variation_args = [
							'post_type'       => 'product_variation',
							'post_status'     => 'publish',
							'posts_per_page'  => -1,
							'fields'          => 'ids',
							'post_parent__in' => $parent_ids_for_variations,
						];

						if ( ! empty( $product_status ) && ! in_array( 'all', $product_status, true ) ) {
							$variation_args['post_status'] = $product_status;
						}

						// Apply the same date filters to variations if applicable
						if ( $export_mode !== 'all' && ! empty( $args['date_query'] ) ) {
							$variation_args['date_query'] = [
								[
									'column'    => $export_mode === 'modified' ? 'post_modified_gmt' : 'post_date_gmt',
									'after'     => $args['date_query'][0]['after'],
									'before'    => $args['date_query'][0]['before'] ?? null,
									'inclusive' => true,
								],
							];
							$this->debug_log( 'YWCE: Variation date query: ' . print_r( $variation_args['date_query'], true ) );
						}

						$this->debug_log( 'YWCE: Variation query args: ' . print_r( $variation_args, true ) );
						$variation_ids = get_posts( $variation_args );
						
						if ( ! is_wp_error( $variation_ids ) ) {
							$this->debug_log( 'YWCE: Found ' . count( $variation_ids ) . ' variations' );
							$item_ids = array_merge( $item_ids, $variation_ids );
							$total_items = count( $item_ids );

							// Reorder so each variable (parent) appears before its variations for readability across all formats
							$variation_map = [];
							foreach ( (array) $variation_ids as $vid ) {
								$parent = (int) get_post_field( 'post_parent', $vid );
								if ( $parent ) {
									if ( ! isset( $variation_map[ $parent ] ) ) {
										$variation_map[ $parent ] = [];
									}
							...
							// Keep the original product order, and insert each product's variations right after the parent
							foreach ( (array) $product_ids as $pid ) {
								$ordered_ids[] = $pid;
								$seen[ $pid ] = true;
								if ( isset( $variation_map[ $pid ] ) ) {
							...
							// Append any remaining variations whose parent isn't in the product list (due to filters)
							if ( ! empty( $variation_map ) ) {
								foreach ( $variation_map as $parent_id => $vids ) {
									foreach ( $vids as $vid ) {
										if ( empty( $seen[ $vid ] ) ) {
											$ordered_ids[] = $vid;
											$seen[ $vid ] = true;
							...
							$this->debug_log( 'YWCE: Error getting variations: ' . $variation_ids->get_error_message() );
						}
					}
				}
				...
				$item_ids = $user_query->get_results();
				$total_items = count($item_ids);
				...
				if (is_wp_error($order_ids)) {
					wp_send_json_error(['message' => 'Error retrieving orders: ' . $order_ids->get_error_message()]);
					return;
				}
			...
			$export_data = [
				...
				'item_ids'        => $item_ids,
				'headers'         => array_merge($fields, $meta_fields, $taxonomies),
				'batch_number'    => 0
			];
	...
	public function process_export(): void {
		...
		try {
			...
			// Check if current format is completed
			if ($processed >= $total) {
				// Finalize the current format if needed
				if (in_array($current_format, ['xml', 'excel'])) {
