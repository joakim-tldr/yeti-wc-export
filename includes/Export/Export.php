				...
				if ( $export_mode !== 'all' ) {
					...
					switch ( $date_range ) {
						case 'last30':
							...
							break;
						case 'last60':
							$date_query = [ 'after' => date( 'Y-m-d', strtotime( '-60 days' ) ), 'inclusive' => true ];
							break;
						case 'last90':
							$date_query = [ 'after' => date( 'Y-m-d', strtotime( '-90 days' ) ), 'inclusive' => true ];
							break;
						case 'custom':
							if ( $date_from ) {
			...
			if ( $processed === 0 && ! empty( $export_items ) ) {
				...
				if ( ! empty( $custom_headers ) ) {
					...
					foreach ( $export_data['headers'] as $header ) {
						$header_mapping[ $header ] = $custom_headers[ $header ] ?? $header;
					}
